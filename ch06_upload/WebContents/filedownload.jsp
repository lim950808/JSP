...
<%
	...
	try {
		...
		client = request.getHeader("User-Agent"); //브라우저 종류 확인
		//파일 다운로드 헤더 지정
		response.reset();
		...
		if(!skip) {
			//IE
			if(client.indexOf("MSIE") != -1) { //internetExplorer 브라우저인 경우
			...
			out.print("<script>alert('파일을 찾을 수 없습니다.'); histroy.back(); </script>");
		}
		in.close(); os.close();
	}catch(Exception e) {
		e.printStackTrace();
